...
namespace Abstraction_Polymorphism_Demo
{
    class Program
    {
        ...
        private static void ShapesDemo()
        {
            ...
            Triangle tr = new Triangle(5, 12);
            Console.WriteLine(tr);

            Parallelogram pa = new Parallelogram(3, 5);
            ...
            do
            {
                choice = GetPromptedInput("What type of shape do you want (circle, square, Triangle, Parallelogram, or Q to quit)?").ToUpper();
                switch (choice)
                {
                    ...
                    // Get the measurements for a new Triangle and add them to the list
                    case "TRIANGLE":
                        shapes.Add
                        (
                            new Triangle
                            (
                                GetPromptedPositiveDouble("What is the base length?"),
                                GetPromptedPositiveDouble("What is the height?")
                            )
                            );
                        break;

                    // Get the measurements for a new Parallelogram and add them to the list
                    case "PARALLELOGRAM":
                        shapes.Add
                        (
                            new Parallelogram
        ...
        private static void PetsDemo()
        {
            ...
            string[] names = 
            {
                "Shiro", "Odin", "Donald", "Odysseus", "Wolfgang", "Samson", "Bucky", "Fido", "Gilgamesh", "Bojack", "Spot"
            ...
            for (int i = 0; i < pets.Length; i++)
            {
                ...
                {
                    pets[i] = new Goldfish(names[i], DateTime.Today);
                }
                else // i % 3 must be 4 in this case
                {
                    pets[i] = new Horse(names[i], DateTime.Today);
                }
                pets[i].Speak();
            }
            Console.WriteLine();

            // Loop through the array of PETS trying to call pet-type specific methods
            for (int i = 0; i < pets.Length; i++)
            {
                ...
                else if (pets[i] is Horse) 
                {
                    // Downcase to Horse to ensure the InvadeTroy method can run
                    ((Horse)pets[i]).InvadeTroy();
